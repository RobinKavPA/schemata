...
def mapl_accessibility_formatAndStandards_vocabularyEncodingScheme(dm_v1, dm_v2):
    ...
    if dm_controlledVocabulary:
        ...
        for e in encodingSchemes:
            ...
            dm_v2["accessibility"]["formatAndStandards"]["vocabularyEncodingScheme"].append(e)


def mapl_accessibility_formatAndStandards_conformsTo(dm_v1, dm_v2):
    dm_conformsTo = dm_v1.get('conformsTo', None)
    dm_v2["accessibility"]["formatAndStandards"]["conformsTo"] = []
    if dm_conformsTo:
        ...
        for c in conformsTo:
            ...
            if 'AMT' == c:
                ...
                c = 'SPL'
            dm_v2["accessibility"]["formatAndStandards"]["conformsTo"].append(c)
...
def mapl_accessibility_formatAndStandards_language(dm_v1, dm_v2):
    ...
    if 'English (UK)'==dm_language:
        dm_language='en'
    dm_v2["accessibility"]["formatAndStandards"]["language"] = []
    if dm_language:
        dm_v2["accessibility"]["formatAndStandards"]["language"] = [dm_language]


def mapl_accessibility_formatAndStandards_format(dm_v1, dm_v2):
    dm_v2["accessibility"]["formatAndStandards"]["format"] = []
    dm_format = dm_v1.get('format', None)
    if dm_format:
        dm_format = dm_format.replace(';', ',')
        formats = dm_format.split(',')
        for f in formats:
            dm_v2["accessibility"]["formatAndStandards"]["format"].append(f.lstrip())


def mapl_enrichmentAndLinkage_qualifiedRelation(dm_v1, dm_v2):
    dm_v2["enrichmentAndLinkage"]["qualifiedRelation"] = []
    ...
    if dm_linkedDataset:
        ...
        for l in linkedDatasets:
            dm_v2["enrichmentAndLinkage"]["qualifiedRelation"].append(l.lstrip())


def mapl_enrichmentAndLinkage_derivation(dm_v1, dm_v2):
    dm_v2["enrichmentAndLinkage"]["derivation"] = []
    ...
    if dm_derivedDatasets:
        ...
        for d in derivedDatasets:
            dm_v2["enrichmentAndLinkage"]["derivation"].append(d.lstrip())


def mapl_enrichmentAndLinkage_tools(dm_v1, dm_v2):
    dm_v2["enrichmentAndLinkage"]["tools"] = [] #['enrichmentAndLinkage_tools']
